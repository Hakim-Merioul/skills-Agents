#!/usr/bin/env node
/**
 * validate-deck.mjs — Deck validator.
 *
 * Lints a Swiss-template HTML deck against the 22 locked layouts (S01-S22),
 * font/color rules, and image-ratio constraints. Exits non-zero on any error.
 *
 * USAGE:
 *   node validate-deck.mjs <index.html> [--allow-experimental]
 *
 * CHECKS:
 *   - Each <section class="slide"> must have a registered data-layout (S01-S22,
 *     SWISS-COVER-ASCII, SWISS-CLOSING-ASCII)
 *   - No text-align:center on non-statement slide titles
 *   - No <text> elements inside inline SVG
 *   - All local images must declare data-image-slot
 *   - S22 hero images must use the s22-hero-21x9 slot
 *   - S15/S16 21:9 grid images must use .frame-img.r-21x9
 *
 * Exits 0 on pass, 1 on validation failure, 2 on usage error.
 */
